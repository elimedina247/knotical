...
public partial class BoatWake : Node
{
    ...
    public float TrailWidth => _hull?.BeamWidth ?? 10f;
    ...
    public override void _Ready()
    {
        ...
        _hull = _body as IDeckBody;
    }

    public override void _Process(double delta)
    {
        Knotical.Ocean.Ocean ocean = Knotical.Ocean.Ocean.Instance;
        if (_hull == null || _body == null || ocean == null) return;
        ...
        Basis basis = _body.GlobalBasis.Orthonormalized();
        var ahead = new Vector2(-basis.Z.X, -basis.Z.Z);
        float heading = Mathf.Atan2(ahead.X, ahead.Y);

        Vector3 bow = _body.ToGlobal(new Vector3(0f, 0f, -HullLength * 0.5f));
        Bow = new Vector4(bow.X, bow.Z, heading, fraction);

        if (speed < MinSpeed) return;

        Vector3 stern = _body.ToGlobal(new Vector3(0f, 0f, HullLength * 0.5f));
