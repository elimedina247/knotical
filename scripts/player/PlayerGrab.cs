...
public partial class PlayerGrab : Node3D
{
	...
	public float CarryMassRatio { get; set; } = 8f;
	...
	public float PushReach { get; set; } = 1.1f;

	[Export(PropertyHint.Range, "5,400,1")]
	public float PushImpulse { get; set; } = 140f;

	[Export(PropertyHint.Range, "1,30,0.5")]
	public float PushSpeedCap { get; set; } = 9f;

	[Export(PropertyHint.Range, "0,1,0.01")]
	public float PushRecoil { get; set; } = 0.35f;

	[Export(PropertyHint.Range, "0.1,2,0.05")]
	public float PushCooldown { get; set; } = 0.5f;

	[Export(PropertyHint.Range, "0.05,0.6,0.01")]
	public float PushTime { get; set; } = 0.22f;

	[Export(PropertyHint.Range, "0,1,0.01")]
	public float LazyReach { get; set; } = 0.6f;
	...
	public override void _Ready()
	{
		...
		_pushMapped = InputMap.HasAction("grab_right");
	...
	public override void _PhysicsProcess(double delta)
	{
		...
		_pushWait = Mathf.Max(_pushWait - dt, 0f);
		_pushing = Mathf.Max(_pushing - dt, 0f);
		if (!held) _rearm = false;

		bool push = _pushMapped ? Input.IsActionPressed("grab_right") : Input.IsMouseButtonPressed(MouseButton.Right);
		bool shove = push && !_wasPush;
		_wasPush = push;

		if (shove && _pushWait <= 0f && (_carrier == null || !_carrier.IsBusy))
		{
			if (!_attached) Push();
			else if (_carried != null) Throw();
		}
	...
	private void Catch()
	{
		...
		else if (node is RigidBody3D { Freeze: false } rigid && rigid.Mass < OwnMass() * CarryMassRatio)
		{
			_carried = rigid;
	...
	private void Pose(bool held, float dt)
	{
		if (!_attached || !IsInstanceValid(_node))
		{
			if (_pushing > 0f)
			{
				Vector3 side = _pivot.GlobalBasis.X * (HandSpread * 0.5f);
				Mark(null);
				ArmLeft?.Grip(_pushAt - side);
				ArmRight?.Grip(_pushAt + side);
				return;
			}
			...
			Mark(tip);
			ArmRight?.Grip(chest.Lerp(tip, LazyReach));
			return;
		}
		...
		if (_carried != null && IsInstanceValid(_carried)) anchor = CarryAnchor(out normal);
		else if (!Hold(out anchor)) anchor = _node.GlobalTransform * _local;
	...
	private Vector3 CarryAnchor(out Vector3 normal)
	{
		Vector3 chest = ChestWorld();
		Vector3 center = CenterOfMass(_carried);
		Vector3 back = chest - center;
		normal = back.LengthSquared() > 1e-6f ? back.Normalized() : Vector3.Up;

		_cast.GlobalTransform = new Transform3D(Basis.Identity, chest);
		_cast.TargetPosition = center - chest;
		_cast.ForceShapecastUpdate();

		float nearest = float.MaxValue;
		Vector3 point = center + normal * CastRadius;

		for (int i = 0; i < _cast.GetCollisionCount(); i++)
		{
			if (_cast.GetCollider(i) != _carried) continue;

			float distance = chest.DistanceSquaredTo(_cast.GetCollisionPoint(i));
			if (distance >= nearest) continue;

			nearest = distance;
			point = _cast.GetCollisionPoint(i);
			normal = _cast.GetCollisionNormal(i);
		}
	...
	private void Push()
	{
		_pushWait = PushCooldown;
		_pushing = PushTime;

		Vector3 chest = ChestWorld();
		Vector3 aim = Aim(chest);
		_pushAt = chest + aim * PushReach;

		if (!Sweep(chest, _pushAt, out Node3D node, out Vector3 point, out _)) return;

		_pushAt = point;

		float impulse = PushImpulse;
		if (node is RigidBody3D { Freeze: false } rigid)
		{
			impulse = Mathf.Min(PushImpulse, rigid.Mass * PushSpeedCap);
			rigid.ApplyImpulse(aim * impulse, point - CenterOfMass(rigid));
		}

		_body?.ApplyCentralImpulse(aim * (-impulse * PushRecoil));
	}

	private void Throw()
	{
		if (!IsInstanceValid(_carried)) return;
		...
		Vector3 chest = ChestWorld();
		Vector3 aim = Aim(chest);
		RigidBody3D thrown = _carried;
		_pushAt = chest + aim * PushReach;

		Drop();

		float impulse = Mathf.Min(PushImpulse, thrown.Mass * PushSpeedCap);
		thrown.ApplyCentralImpulse(aim * impulse);
		_body?.ApplyCentralImpulse(aim * (-impulse * PushRecoil));
	}
	...
	private void Carry(PhysicsDirectBodyState3D state, Vector3 chest)
	{
		...
		_load = Mathf.Clamp(_carried.Mass / (OwnMass() * CarryMassRatio), 0f, 1f);
	}
