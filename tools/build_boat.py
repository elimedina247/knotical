...
out_glb = os.path.abspath(args[0] if args else "models/boat_toon.glb")
out_json = os.path.abspath(args[1] if len(args) > 1 else "models/boat_toon.json")
...
def build():
    ...
    paint.add_face(list(reversed(transom)), cream, False)
    tz = [paint.verts[v][2] for v in shell_rings[0]]
    stripe_c = paint.add_vert((0.0, station_y(0.0) - TRANSOM_RAKE * 0.55 - 0.01, (STRIPE_LO + STRIPE_HI) * 0.5))

    bow = bul[-1]
    stem_cap = [bow[(1, "inner_bottom")], bow[(1, "inner_top")], bow[(1, "outer_top")], bow[(1, "outer_cap")],
                bow[(1, "sheer")], bow[(-1, "sheer")], bow[(-1, "outer_cap")], bow[(-1, "outer_top")],
                bow[(-1, "inner_top")], bow[(-1, "inner_bottom")]]
    paint.add_face(stem_cap, cream, False)
    ...
    profile = [(-cabin_w, 0.0), (-cabin_w, cabin_h - roof_r)]
    ...
    for i in range(len(profile) - 1):
        ...
        if 0.55 <= max(z0, z1) <= 0.8:
            color = teal
        paint.add_face([ring_a[i], ring_b[i], ring_b[i + 1], ring_a[i + 1]], color, min(z0, z1) >= cabin_h - roof_r - 1e-6)
    ...
    for side in (-1, 1):
        ...
        paint.add_face(quad if side > 0 else list(reversed(quad)), red, False)
    ...
    spec = {
        ...
        "mast_y": 1.2, "helm_y": round(STERN_Y + 2.1, 3), "halyard": [0.55, 1.5, MAIN_DECK + 1.6],
...
def main():
    ...
    kwargs = dict(filepath=out_glb, export_format="GLB", export_apply=True, export_yup=True,
    ...
    print("boat: wrote %s and %s" % (out_glb, out_json))
